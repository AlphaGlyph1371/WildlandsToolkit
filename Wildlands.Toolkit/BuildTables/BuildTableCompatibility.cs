...
public static class BuildTableCompatibility
{
    ...
    public static BuildTableSlot DetectSlot(string value)
    {
        ...
        // BuildTable names usually start with the thing they select and end in
        // compatibility conditions (for example Top_NoVests_noHat). Walk in name
        // order so those conditions do not replace the table's actual purpose.
        for (int i = 0; i < words.Length; i++)
        {
            ...
            if (word is "underbarrel" or "underbarrels" or "foregrip" or "foregrips"
                || word == "under" && next is "barrel" or "barrels")
                return BuildTableSlot.Underbarrel;
            if (word is "siderail" or "siderails"
                || word == "side" && next is "rail" or "rails")
                return BuildTableSlot.SideRail;
            if (word is "flashhider" or "flashhiders"
                || word == "flash" && next is "hider" or "hiders"
                || word is "muzzle" or "muzzles" or "suppressor" or "suppressors"
                    or "silencer" or "silencers")
                return BuildTableSlot.Muzzle;
            if (word is "magazine" or "magazines" or "mag")
                return BuildTableSlot.Magazine;
            if (word is "scope" or "scopes" or "optic" or "optics" or "sight" or "sights"
                or "ironsight" or "ironsights"
                || word == "iron" && next is "sight" or "sights")
            ...
            if (word is "beard" or "beards" or "facialhair"
                || word == "facial" && next == "hair")
            ...
            if (word is "vest" or "vests" or "tacvest" or "tacticalvest"
                || word is "tac" or "tactical" && next is "vest" or "vests")
